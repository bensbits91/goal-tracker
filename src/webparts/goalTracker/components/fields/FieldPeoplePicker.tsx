...
const mcc = 'color:black;background-color:pink;';
...
class FieldPeoplePicker extends React.Component<FieldPeoplePickerProps, FieldPeoplePickerState> {
    ...
    private _getPeoplePickerItems(items: any[]) {
        console.log('%c : FieldPeoplePicker -> _getPeoplePickerItems -> items', mcc, items);
        let item = items[0];
        item.field = 'Salesperson';
        item.value = item.secondaryText;
        this.props.handler(item);
    }

    public render() {
        const { field, context } = this.props;
        return (
            <PeoplePicker
                context={context}
                titleText="Sold By"
                ...
                resolveDelay={1000}
                defaultSelectedUsers={[field.value]}
            />
        );
    }
}
...
function Builder(props) {
    console.log('%c : Builder -> props', mcc, props);
    const [mostRecentlyUsed, setMostRecentlyUsed] = React.useState<IPersonaProps[]>(mru);
    const [peopleList, setPeopleList] = React.useState<IPersonaProps[]>(people);
    console.log('%c : FieldPeoplePicker:React.FunctionComponent -> peopleList', mcc, peopleList);
...
//     console.log('%c : FieldPeoplePicker:React.FunctionComponent -> props', mcc, props);
//     const [mostRecentlyUsed, setMostRecentlyUsed] = React.useState<IPersonaProps[]>(mru);
//     const [peopleList, setPeopleList] = React.useState<IPersonaProps[]>(people);
//     console.log('%c : FieldPeoplePicker:React.FunctionComponent -> peopleList', mcc, peopleList);
